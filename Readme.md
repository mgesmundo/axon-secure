# Axon-Secure
  ...
  Axon-Secure is a fork of [Axon](https://www.npmjs.org/package/axon), a message-oriented socket library for node.js heavily inspired by zeromq. For a light-weight   UDP alternative you may be interested in [punt](https://github.com/visionmedia/punt).

## Differences with Axon

  Axon-Secure has some differences with the original Axon project:

  - a new security plugin is introduced to enable encryption for the messages
  - fixed round-robin plugin
  - the unix domain support is drop
  - code cleanup
  - updated dependencies

*NOTE*: the initial version of this plugin is set to 2.0.0 because this is a fork of a 2.0.0 version of Axon.

## Installation

    $ npm install axon-secure
...
In all patterns you can enable the encryption for all messages. For example in **push / pull** pattern:
...
Every socket has associated options that can be configured via `get/set`.
  ...
  - `secure` - set `true` to encrypt every message (default is `false`)
  - `cipher` - the cipher used for encryption (default is `aes256`)
  - `secret` - the shared password for encryption (default is `secret`: **remember to change it!**).
